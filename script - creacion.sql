...
CREATE TABLE [POLACA_INVERSA].ITEMS_FACTURAS_RENDIDAS(
	ID_RENDICION INT FOREIGN KEY REFERENCES POLACA_INVERSA.FACTURAS_RENDIDAS(ID_RENDICION),
	ID_FACTURA NUMERIC(18,0) FOREIGN KEY REFERENCES POLACA_INVERSA.FACTURAS(ID_FACTURA),
...
BEGIN
	...
	SELECT DISTINCT [Pago_nro],[Pago_Fecha],[Total],(select ID_MEDIO FROM POLACA_INVERSA.MEDIOS_PAGO WHERE DESCRIPCION=FormaPagoDescripcion),(select ID_SUCURSAL from POLACA_INVERSA.SUCURSALES WHERE CODIGO_POSTAL = Sucursal_Codigo_Postal), (select ID_CLIENTE FROM POLACA_INVERSA.CLIENTES where DNI = [Cliente-Dni])  
